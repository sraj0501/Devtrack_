# Repository metadata recommendation

Draft for TASK-122 review. This file records the exact GitHub metadata copy; it does not
change the repository or publish anything.

## Description

> Never write a standup again. DevTrack turns Git commits into staged ticket updates and end-of-day reports, local-first and in your voice.

Length: 137 characters. The copy leads with the released user outcome and names the trust
boundary (`staged`) without advertising the unreleased MCP command.

## Topics

Use this exact ordered set:

```text
developer-tools
productivity
automation
git
cli
golang
local-first
offline-first
mcp
model-context-protocol
ollama
github
gitlab
jira
azure-devops
```

The first ten topics describe the product and its differentiators. The remaining five
cover its local inference and project-management integration surfaces. All topics are
lowercase and hyphenated for GitHub compatibility.

## Review boundary

- Latest public release remains `v3.0.10`; TASK-118–120 are unreleased work on `dev`.
- `mcp` and `model-context-protocol` are repository-code topics, not a claim that the
  `v3.0.10` release binary contains `devtrack mcp`; defer MCP description copy until a
  release containing that command is published.
- No usage, time-saved, customer-count, or adoption claim is included.
- Applying the description or topics requires separate explicit authorization.
