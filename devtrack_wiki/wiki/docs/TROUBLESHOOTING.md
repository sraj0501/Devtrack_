...
### "error: DEVTRACK_SERVER_URL is required"

This warning appears when the client is configured to use the Python server but the URL is not set. Set it in your `.env` or environment only if you have `devtrack_server` running:

```bash
export DEVTRACK_SERVER_URL=https://localhost:8089
```
...
If you are running the client standalone (no server), you can ignore this warning — the binary works without it.

### "connection refused" or "cannot connect to devtrack"
...
# Verify it's listening on port 8765
# Check DEVTRACK_SERVER_HTTP_PORT in server .env
```

### "error: unknown command"

The CLI supports: `start`, `stop`, `status`, `logs`, `pause`, `resume`, `force-trigger`, `version`, `health`.
Commands like `git commit`, `standup`, `stats`, `restart` are not CLI commands — they run on the server directly via `devtrack <command>`.
...
4. Restart daemon:
```bash
devtrack stop && devtrack start  # no restart command — stop then start
```
...
```bash
file ~/.local/bin/devtrack  # Should show "Mach-O 64-bit" on macOS, "ELF 64-bit" on Linux
