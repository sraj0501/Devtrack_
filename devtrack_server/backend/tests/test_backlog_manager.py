...
def bm(db):
    ...
    yield BacklogManager(engine=db)
...
def test_add_item_returns_backlog_item(bm, project):
...
def test_add_item_persists(bm, project):
...
def test_add_item_defaults(bm, project):
...
def test_add_item_custom_fields(bm, project):
    item = bm.add_item(
        project, "Fix crash",
        item_type="bug",
        priority="critical",
...
def test_list_items_empty(bm, project):
    assert bm.list_items(project) == []


def test_list_items_returns_all(bm, project):
...
def test_list_items_filter_status(bm, project):
...
def test_update_item(bm, project):
...
def test_backlog_returns_unassigned(bm, project):
    sprint = bm.create_sprint(project, "Sprint 1")
...
def test_create_sprint(bm, project):
...
def test_get_sprint(bm, project):
    sprint = bm.create_sprint(project, "Sprint 1")
    fetched = bm.get_sprint(sprint.id)
    assert fetched.id == sprint.id


def test_get_sprint_missing(bm):
    assert bm.get_sprint("nope") is None


def test_list_sprints(bm, project):
...
def test_list_sprints_filter_status(bm, project):
...
def test_update_sprint(bm, project):
    sprint = bm.create_sprint(project, "Sprint 1")
...
def test_close_sprint_tallies_points(bm, project):
    sprint = bm.create_sprint(project, "Sprint 1")
...
def test_average_velocity_no_closed(bm, project):
    bm.create_sprint(project, "Sprint 1")
    assert bm.average_velocity(project) is None


def test_average_velocity(bm, project):
...
def test_estimate_story_points_mocked(bm, project):
    mock_provider = MagicMock()
    mock_provider.generate.return_value = "5"
    with patch("backend.backlog_manager.get_provider", return_value=mock_provider):
...
def test_estimate_story_points_snaps_to_fibonacci(bm, project):
    mock_provider = MagicMock()
...
def test_estimate_story_points_no_llm(bm, project):
    with patch("backend.backlog_manager.get_provider", side_effect=Exception("no llm")):
        item = bm.add_item(project, "Item")
        pts = bm.estimate_story_points(item.id)
    assert pts is None
