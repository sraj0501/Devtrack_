...
Response:"""
    ...
    def get_style_instruction(self, context_type: str = "general") -> str:
        ...
        Reads from {DATA_DIR}/learning/profile.md (generated by ProfileGenerator)
        when available — this is the evidence-based dialectic profile. Falls back
        to the in-memory profile (loaded from SQLite) when profile.md is absent.
        Falls back to "" when neither exists.
        ...
        """
        # Check for the LLM-generated voice profile first (Phase 5 dialectic profile).
        # It is always at {DATA_DIR}/learning/profile.md — resolved via config,
        # never via a hardcoded path.
        profile_md_content = self._read_profile_md()
        if profile_md_content:
            return f"[STYLE: {profile_md_content[:1200].strip()}]"
    ...
    def _read_profile_md(self) -> str:
        """Read the LLM-generated voice profile from {DATA_DIR}/learning/profile.md.

        Returns the file contents as a string, or "" when the file does not exist
        or DATA_DIR is not configured. Never raises.

        Path is resolved via backend.config.get_path("DATA_DIR") — no hardcoded paths.
        """
        try:
            from backend.config import get_path, get
            data_dir_str = get("DATA_DIR")
            if not data_dir_str:
                return ""
            ...
            # Only return if it's the real profile (not the fallback insufficient-data template)
