...
func NewMessageQueue(db *db.Database, ipcServer *infra.IPCServer) *MessageQueue {
	return &MessageQueue{
		db:            db,
		ipcServer:     ipcServer,
		drainInterval: time.Duration(config.GetQueueDrainIntervalSecs()) * time.Second,
		maxRetries:    config.GetQueueMaxRetries(),
...
func (mq *MessageQueue) SendOrQueue(msg infra.IPCMessage) error {
	...
	// If error is not infra.ErrNoClients, it's a different problem
	if err != infra.ErrNoClients {
...
func (mq *MessageQueue) Enqueue(msg infra.IPCMessage) error {
	payload, err := json.Marshal(msg)
	if err != nil {
		return fmt.Errorf("failed to marshal message for queue: %w", err)
	}

	record := db.QueuedMessage{
...
func (mq *MessageQueue) DrainOnce() {
	...
	retentionDays := config.GetQueueRetentionDays()
