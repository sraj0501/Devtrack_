...
func (cli *CLI) handleSettings() error {
	...
	fmt.Printf("  Ollama model: %s\n", getEnvOrDefault("OLLAMA_MODEL", "(not set)"))
	fmt.Println()
...
func (cli *CLI) printUsage() {
	...
	fmt.Println()
	fmt.Println("GIT:")
	fmt.Println("  git commit                       Native Git; observed silently in background")
	fmt.Println("  devtrack git commit -m '<msg>'   Explicit AI-enhanced commit message")
	fmt.Println("    A  Accept  |  E  Enhance further  |  R  Regenerate  |  Q  Defer")
	fmt.Println("    Returns after Git commits; no ticket, time, PM-post, or push questions")
	...
	fmt.Println(`  eval "$(devtrack shell-init)"    Optional git history/messages aliases`)
	fmt.Println("  devtrack enable-git              Enable silent observation for this repo")
	...
	fmt.Println("    devtrack github-sync                     Sync issues → local cache + AI server")
	fmt.Println()
	...
	fmt.Println("    devtrack gitlab-sync                     Sync issues → local cache + AI server")
	fmt.Println()
	...
	fmt.Println("    devtrack azure-sync                      Sync work items → local cache + AI server")
	fmt.Println()
	...
	fmt.Println()
	fmt.Println("  Ticket Sync  (all enabled PM platforms at once)")
	fmt.Println("    devtrack ticket-sync             Sync all workspaces → local cache + AI server")
	fmt.Println("    devtrack ticket-sync --force     Force drop+reload of AI server cache")
	fmt.Println("    Sync happens automatically on daemon start (TICKET_SYNC_ON_START) and")
	fmt.Println("    every N hours in the background (TICKET_SYNC_INTERVAL_HOURS).")
	fmt.Println("    AI server push failures are non-fatal: local cache always written first.")
	fmt.Println()
