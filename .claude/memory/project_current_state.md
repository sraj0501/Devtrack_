...
description: v3.0.10; Phases 0–8 and PostgreSQL epic complete; Phase 9 active; TASK-119 in progress
type: project
---

**Version:** v3.0.10. As of 2026-08-18, `origin/dev` is `1edf303` after TASK-118 PR #254 and remains ahead of `origin/main`. GitHub `sraj0501/Devtrack_` is the sole source. Release: `.\scripts\release.ps1 [-Bump patch|minor|major]`.
...
**Recent merges:** TASK-141 PR #247; TASK-114 PR #249; TASK-115 PR #250; TASK-116 PR #251; TASK-117 PR #252; TASK-142 PR #253; TASK-118 PR #254. TASK-118 made managed checkout, `uv sync`, and local model pulls detached and observable through durable `status`/`doctor` state.

**Task-ID ledger:** `Data/agent_logs/project_board.md` is authoritative. TASK-119 is active on `features/TASK-119-first-run-wow`; next unused ID: **TASK-143**.

**Active next:** Phase 9 — Adoption Gate — TASK-117–124 (`docs/NEXT_STEPS.md`). TASK-117, TASK-142, and TASK-118 are complete. TASK-119 adds the dependency-free MCP first-run prompt and a non-blocking, one-time local Git voice seed/profile worker; TASK-120 follows.
