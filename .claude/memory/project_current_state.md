...
description: v3.0.10; Phases 0–8 and PostgreSQL epic complete; Phase 9 active; TASK-142 planned before TASK-118
type: project
---

**Version:** v3.0.10. As of 2026-08-18, `origin/dev` is `197c079` and remains ahead of `origin/main`. GitHub `sraj0501/Devtrack_` is the sole source. Release: `.\scripts\release.ps1 [-Bump patch|minor|major]`.
...
**Recent merges:** TASK-141 PR #247; TASK-114 PR #249; TASK-115 PR #250; TASK-116 PR #251; TASK-117 PR #252. TASK-117 writes visible managed-install runtime defaults, preserves valid overrides, and rejects invalid ones. The final deployment/setup PRs passed all seven CI checks.

**Task-ID ledger:** `Data/agent_logs/project_board.md` is authoritative. TASK-142 is planned to replace the legacy `nlp_parser.py`/`NLPTaskParser` surface with an explicitly LLM-owned structured parser and non-blocking raw/template fallback. Next unused ID: **TASK-143**.

**Active next:** Phase 9 — Adoption Gate — TASK-117–124 (`docs/NEXT_STEPS.md`). TASK-117 is complete; sequence TASK-142 before TASK-118 so background bootstrap does not preserve stale spaCy assumptions.
