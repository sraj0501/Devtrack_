...
description: gitsage (Go-native), RAG personalization, and Azure DevOps integration quick reference
type: reference
---

## gitsage (Go-native, `devtrack_client/gitsage/`)

Python git_sage removed. Go gitsage is the only implementation.
Approval modes: `[a]` auto / `[r]` review / `[s]` suggest-only. `--yes` skips. Up to 5 follow-ups after task. `undo [N]` = `git reset --hard <pre-step-HEAD>`.
