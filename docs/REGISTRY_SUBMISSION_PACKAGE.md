...
| Glama | **Container-ready; owner submission required** | GitHub OAuth by a maintainer with write/admin access; Glama clones, builds, runs, and introspects the repository using a Dockerfile supplied in its submission UI. | [Glama registry](https://glama.ai/mcp/servers) and [indexing methodology](https://glama.ai/mcp/methodology) | Submit `https://github.com/sraj0501/Devtrack_` and paste the repository's `Dockerfile.mcp` as the Glama Dockerfile. It builds the Go binary, starts the local stdio MCP server, and creates a disposable SQLite database solely for initialization and tool introspection. DevTrack uses a source-available Community License, so the owner must accept any eligibility terms shown by Glama. Do not submit the Python backend Dockerfile or present the server as hosted/remote. |
...
| `punkpeye/awesome-mcp-servers` | **Submitted; checks pass** | A repository-linked server name, concise accurate description, appropriate category, alphabetical placement, one server per line, and a contributor PR. | [PR #13608](https://github.com/punkpeye/awesome-mcp-servers/pull/13608) and [contribution guide](https://github.com/punkpeye/awesome-mcp-servers/blob/main/CONTRIBUTING.md) | Submitted under Developer Tools on 2026-09-04 after rechecking the live rules. The automated `check-submission` job passed; maintainer review/merge remains external. |
...
```markdown
- [sraj0501/Devtrack_](https://github.com/sraj0501/Devtrack_) 🏠 🏎️ - Local stdio server exposing six read-only tools for active-ticket, commit, pending-action, voice-profile, ticket, and EOD context from DevTrack's SQLite database. [![sraj0501/Devtrack_ MCP server](https://glama.ai/mcp/servers/sraj0501/Devtrack_/badges/score.svg)](https://glama.ai/mcp/servers/sraj0501/Devtrack_)

Do not add the badge to the live PR until the Glama listing exists and its checks pass. If Glama
normalizes the repository slug differently, use the exact path shown on the successful listing
instead of assuming `sraj0501/Devtrack_`.
```
